...
- těleso $\R$ s příslušnými operacemi, dán lineární prostor $\L$ nad tělesem $\R$, prvky v $\L$ vektory, $+: \L^2 \to \L$ (sčítání vektorů), a $\cdot: \R \times \L \to \L$ (škálování vektorů, nebol násobení vektorů skalárem), splňující dané axiomy:
	- vlastnosti $+$
